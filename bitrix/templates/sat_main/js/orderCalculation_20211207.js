window.addEventListener('load', () => {
        ...
        out += `
          <div class="options-row-item options-body-item">
            ...
            <div class="price-total">
              <span class="text">${api_lang === 'ru' ? 'цена' : 'ціна'}: </span>
              <span class="value">0 грн.</span>
            </div>

            <div class="price-total">
              <span class="text">${api_lang === 'ru' ? 'итого' : 'загалом'}: </span>
              <span class="value">0 грн.</span>
            </div>

            <div class="qty-wrap">
              <input type="text" value="1" placeholder="0" ${disabled} />
              <span>${units}</span>
            </div>
          </div>`;
      });
  ...
  const bntCalcOnClick = async () => {
    ...
    const data = {
      addServices: [],
      cargoType: subtypeOfCargo.options[subtypeOfCargo.selectedIndex].dataset.ref, //идентификатор вида груза
    ...
    if( resData.success === 'true' ) {
      ...
      if (responseData.addServices) {
        ...
        responseData.addServices.forEach(el => {
          // "Укладка груза в обрешетку", "Укладка груза в обрешетку на поддоне"
          if (['456a461e-1bf5-11ea-940d-00505601031c', 'ddf16bac-1bf5-11ea-940d-00505601031c'].includes(el.service.ref)) hasLathing = true;
